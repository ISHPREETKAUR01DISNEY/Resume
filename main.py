...
def create_mask(img):
    ...
    lower = np.array([0,26,44])
    upper = np.array([179,224,255])
    mask = cv2.inRange(imgHSV, lower, upper)
    return mask


def threshold(mask):
    _, thresh = cv2.threshold(mask, 127, 255,
                              cv2.THRESH_BINARY)  # if pixel intensity <= 127 then set it as 0 and pixel intensity > 127 set it as 255
    return thresh


def find_contours(thresh):
    contours, heirarchy = cv2.findContours(thresh, cv2.RETR_EXTERNAL,
...
def max_contour(contours):
    ...
    epsilon = 0.005 * cv2.arcLength(max_cntr,
                                    True)  # maximum distance from contour to approximated contour. It is an accuracy parameter
